...
export const useFormNestRoot = <T>({
  ...
  const [validMap, setValidMap] = useState<Record<string, TmpErrorType | null>>(
    {}
  );
...
export const useArrayNest = <T, P>({
  ...
  const [validMap, setValidMap] = useState<
    Record<string, TmpErrorType | null>[]
  >([]);
